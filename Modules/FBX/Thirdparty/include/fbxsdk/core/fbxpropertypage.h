...
public:
    ...
    inline FbxStringSymbol GetName() const
    {
        return mName;
    }
    ...
    inline void SetLabel(const char* pLabel)
    {
        mLabel = pLabel;
    }
    ...
    inline void SetUserTag(int pUserTag)
    {
        mUserTag = pUserTag;
    }
    inline int GetUserTag() const
    {
        return mUserTag;
    }
...
public:
    ...
    inline FbxPropertyInfo* Get(const FbxPropertyInfo* /*pType*/)
    {
        return mInfo;
    }
    ...
    inline FbxPropertyValue* Get(const FbxPropertyValue* /*pType*/)
    {
        return mValue;
    }
    ...
    inline FbxPropertyConnect* Get(const FbxPropertyConnect* /*pType*/)
    {
        return mConnect;
    }
    ...
    inline FbxPropertyFlags* Get(const FbxPropertyFlags* /*pType*/)
    {
        return &mFlags;
    }
    inline void Set(FbxPropertyFlags pType)
    {
        mFlags = pType;
    }
    inline void Set(FbxPropertyFlags* pType)
...
public:
    ...
    inline FbxInt GetNextId() const
    {
        return mNextId;
    }
    inline FbxInt GetNextIdAndInc()
    {
        return mNextId++;
    }
...
public:
    ...
    template <class T>
    inline T* GetPropertyItem(const T* pItemType, FbxInt pIndex, FbxPropertyPage** pFoundIn = 0) const
    {
    ...
    template <class T>
    inline T* ChangePropertyItemState(const T* pItemType, FbxInt pIndex, FbxPropertyFlags::EInheritType pInheritType)
    {
        ...
        else
        {
            ...
            if (lOwnEntry && FbxPropertyFlags::eInherit == pInheritType)
                return 0;
    ...
    */
    ...
    void InsertEnumValue(FbxInt pId, int pIndex, const char* pStringValue)
    {
        ...
        // Don't make it writeable (Keep it shared)
        if (lPropertyInfo)
            lPropertyInfo->InsertEnumValue(pIndex, pStringValue);
    }
    ...
    void SetEnumValue(FbxInt pId, int pIndex, const char* pStringValue)
    {
        ...
        // Don't make it writeable (Keep it shared)
        if (lPropertyInfo)
            lPropertyInfo->SetEnumValue(pIndex, pStringValue);
    }
    ...
    bool GetSrc(FbxInt pId, int pIndex, FbxConnectionPointFilter* pFilter, FbxPropertyPage** pSrcPage, FbxInt* pSrcId)
    {
        ...
        if (lPropertyConnect && lReferencePage == this)
        {
            ...
            {
                if (pSrcPage)
                    *pSrcPage = lSrc->GetPage();
                if (pSrcId)
                    *pSrcId = lSrc->GetPropertyId();
                return true;
            }
        }
        return false;
    ...
    bool GetDst(FbxInt pId, int pIndex, FbxConnectionPointFilter* pFilter, FbxPropertyPage** pDstPage, FbxInt* pDstId)
    {
        ...
        if (lPropertyConnect && lReferencePage == this)
        {
            ...
            {
                if (pDstPage)
                    *pDstPage = lDst->GetPage();
                if (pDstId)
                    *pDstId = lDst->GetPropertyId();
                return true;
            }
        }
        return false;
    ...
    inline FbxPropertyFlags::EInheritType GetValueInherit(FbxInt pId, bool pCheckInstanceOf) const
    {
        ...
        {
            if (lReferencePage == this)
                return FbxPropertyFlags::eOverride; // this page is either an override, or the originator
            else if (!lReferencePage->mInstanceOf)
                return FbxPropertyFlags::eInherit; // the reference is the class root, so we must be inheriting
    ...
    template <class T>
    inline bool Set(FbxInt pId, const T& pValue)
    {
        return Set(pId, &pValue, FbxTypeOf(pValue), true);
    }
    template <class T>
    inline T Get(FbxInt pId, const T* pFBX_TYPE)
    {
        T lValue;
        Get(pId, &lValue, FbxTypeOf(lValue));
        return lValue;
    }
    ...
    inline const FbxPropertyPage* GetInstanceOf() const
    {
        return mInstanceOf;
    }
    inline FbxPropertyPage* GetInstanceOf()
    {
        return mInstanceOf;
    }

    inline const FbxArray<FbxPropertyPage*>& GetInstances() const
    {
        return mInstances;
    }
    inline FbxArray<FbxPropertyPage*>& GetInstances()
    {
        return mInstances;
    }
...
private:
    ...
    */
    FbxInt GetMinimumPropertyId(FbxInt pId, bool pIncrementIfNone = true) const
    {
        ...
        if (lParentNull && lMinNull)
            return pIncrementIfNone ? pId + 1 : FBXSDK_PROPERTY_ID_NULL;
        else if (lMinNull)
            lMin = lParentMin;
        else if (!lParentNull)
            lMin = lMin < lParentMin ? lMin : lParentMin;
    ...
    */
    ...
    FbxInt Add(FbxInt pParentId, FbxPropertyInfo* pInfo, FbxPropertyValue* pValue, FbxPropertyConnect* pConnect, bool pRecursive = true)
    {
        ...
        if (lFlags)
            lFlags->ModifyFlags(FbxPropertyFlags::eAllFlags, false);
