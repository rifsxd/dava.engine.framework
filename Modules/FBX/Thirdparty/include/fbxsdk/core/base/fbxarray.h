...
public:
    ...
    inline T& operator[](const int pIndex) const
    {
        ...
        if (pIndex >= mSize)
        {
            if (pIndex < mCapacity)
            {
                ...
                FBX_ASSERT_NOW("Index is out of range!");
        }
	#endif
	...
	* \remark If the requested element count is less than or equal to the current count, elements are freed from memory. Otherwise, the array grows and elements are unchanged. */
    ...
    {
        if (pSize == mSize && mSize == mCapacity)
            return true;
...
template <class T>
inline void FbxArrayFree(FbxArray<T>& pArray)
{
...
template <class T>
inline void FbxArrayDelete(FbxArray<T>& pArray)
{
    for (int i = 0, c = pArray.Size(); i < c; ++i)
    {
        FbxDelete(pArray[i]);
...
template <class T>
inline void FbxArrayDestroy(FbxArray<T>& pArray)
{
    for (int i = 0, c = pArray.Size(); i < c; ++i)
    {
        (pArray[i])->Destroy();
