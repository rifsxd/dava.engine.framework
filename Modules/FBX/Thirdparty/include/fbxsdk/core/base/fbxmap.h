...
*****************************************************************************************************************************/
    ...
    inline FbxMap(const FbxMap& pMap)
        : mTree(pMap.mTree)
    {
    }
    inline ~FbxMap()
...
class FBXSDK_DLL FbxObjectMap : public FbxSimpleMap<Type, FbxObject*, Compare>
{
public:
    //! Constructor
    inline FbxObjectMap()
...
inline void FbxMapFree(FbxMap<K, V, C, A>& pMap)
{
...
template <typename K, typename V, typename C, typename A>
inline void FbxMapDelete(FbxMap<K, V, C, A>& pMap)
{
    for (typename FbxMap<K, V, C, A>::Iterator i = pMap.Begin(); i != pMap.End(); ++i)
    {
...
template <typename K, typename V, typename C, typename A>
inline void FbxMapDestroy(FbxMap<K, V, C, A>& pMap)
{
    for (typename FbxMap<K, V, C, A>::Iterator i = pMap.Begin(); i != pMap.End(); ++i)
    {
        i->GetValue()->Destroy();
