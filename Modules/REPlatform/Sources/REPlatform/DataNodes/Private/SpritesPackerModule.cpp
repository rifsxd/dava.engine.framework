...
namespace DAVA
{
SpritesPackerModule::SpritesPackerModule(DAVA::UI* ui_, DAVA::ContextAccessor* accessor_)
    : QObject(nullptr)
...
void SpritesPackerModule::ShowPackerDialog()
{
    ...
    ui->ShowModalDialog(DAVA::mainWindowKey, &dialogReloadSprites);
}
...
void SpritesPackerModule::CreateWaitDialog(const DAVA::FilePath& projectPath)
{
    ...
    waitDialogHandle = ui->ShowWaitDialog(DAVA::mainWindowKey, params);
}
...
void SpritesPackerModule::ReloadObjects()
{
    ...
    if (gpus.empty() == false)
    {
        accessor->GetGlobalContext()->GetData<CommonInternalSettings>()->spritesViewGPU = gpus[0];
    }
