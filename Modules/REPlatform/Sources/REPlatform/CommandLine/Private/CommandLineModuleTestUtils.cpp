...
{
const String SceneBuilder::tagChina = ".china";
const String SceneBuilder::tagJapan = ".japan";
const String SceneBuilder::tagDefault = "";
const String SceneBuilder::chinaSlotDir = "China.slot/";
const String SceneBuilder::defaultSlotDir = "Default.slot/";
...
{
public:
    Impl(const Vector<eGPUFamily>& newLoadingOrder)
    {
        gpuLoadingOrder = Texture::GetGPULoadingOrder();
        Texture::SetGPULoadingOrder(newLoadingOrder);
    }
...
TextureLoadingGuard::TextureLoadingGuard(const Vector<eGPUFamily>& newLoadingOrder)
...
void ClearTestFolder(const FilePath& folder)
{
    ...
    FileSystem::Instance()->DeleteDirectoryFiles(folder, true);
...
void CreateProjectInfrastructure(const FilePath& projectPathname)
{
    ...
    FilePath datasourcePath = projectPathname + "DataSource/3d/";
    FileSystem::Instance()->CreateDirectory(datasourcePath, true);
...
void SceneBuilder::CreateFullScene(const FilePath& scenePath, const FilePath& projectPath, Scene* scene)
{
    ...
    , scene(SafeRetain(scene_))
{
    ...
    FilePath chineseSlotPath = projectPathname + "DataSource/Slot.china.yaml";
    CreateSlotYaml(chineseSlotPath, scenePathname.GetDirectory() + chinaSlotDir + "box_slot.sc2");

    FilePath defaultSlotPath = projectPathname + "DataSource/Slot.yaml";
