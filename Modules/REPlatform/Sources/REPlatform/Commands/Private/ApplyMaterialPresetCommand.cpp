        ...
        */
        ...
        if (nullptr != propData)
        {
            ...
            if (material->HasLocalProperty(propName))
            {
                ...
                uint32 existingSize = material->GetLocalPropArraySize(propName);
...
ApplyMaterialPresetCommand::ApplyMaterialPresetCommand(const FilePath& presetPath, NMaterial* material_, Scene* scene_)
    : RECommand("Apply material preset")
    , redoInfo(new KeyedArchive())
    , undoInfo(new KeyedArchive())
    , material(RefPtr<NMaterial>::ConstructWithRetain(material_))
    , scene(scene_)
{
...
void ApplyMaterialPresetCommand::Init(uint32 materialParts_)
{
...
void ApplyMaterialPresetCommand::StoreCurrentConfigPreset(KeyedArchive* preset, NMaterial* material, const SerializationContext& context)
{
...
void ApplyMaterialPresetCommand::LoadMaterialPreset(KeyedArchive* archive, uint32 parts, bool loadForUndo)
{
    SerializationContext context;
    PrepareSerializationContext(context);
...
void ApplyMaterialPresetCommand::StoreAllConfigsPreset(KeyedArchive* archive, NMaterial* material, const SerializationContext& context)
{
    using namespace ApplyMaterialPresetDetail;
    using namespace DAVA;
...
void ApplyMaterialPresetCommand::PrepareSerializationContext(SerializationContext& context)
{
    ...
    context.SetVersion(VersionInfo::Instance()->GetCurrentVersion().version);
}

DAVA_VIRTUAL_REFLECTION_IMPL(ApplyMaterialPresetCommand)
{
    ReflectionRegistrator<ApplyMaterialPresetCommand>::Begin()
    .End();
}
} // namespace DAVA
