...
bool RadixSort::resize(uint32 nb)
{
    ...
    // Get some fresh one
    mIndices = new uint32[nb];
    CHECKALLOC(mIndices);
    mIndices2 = new uint32[nb];
    CHECKALLOC(mIndices2);
    mCurrentSize = nb;
...
RadixSort& RadixSort::sort(const uint32* input, uint32 nb, bool signedvalues)
{
    ...
    if (!signedvalues)
    {
        CREATE_HISTOGRAMS(uint32, input);
    }
    else
    {
        CREATE_HISTOGRAMS(int32, input);
    }
        ...
        else
        {
            ...
            if (PerformPass)
            {
                ...
                for (i = 0; i < nb; i++)
                {
                    ...
                    if (Radix < 128)
                        mIndices2[mOffset[Radix]++] = mIndices[i]; // Number is positive, same as above
                    else
                        mIndices2[--mOffset[Radix]] = mIndices[i]; // Number is negative, flip the sorting order
                }
                // Swap pointers for next pass. Valid indices - the most recent ones - are in mIndices after the swap.
                uint32* Tmp = mIndices;
                mIndices = mIndices2;
