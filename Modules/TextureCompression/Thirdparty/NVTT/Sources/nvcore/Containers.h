...
private:
    ...
    void allocate(uint rsize)
    {
        ...
        {
            if (m_buffer)
                m_buffer = (T*)mem::realloc(m_buffer, sizeof(T) * m_buffer_size);
            else
                m_buffer = (T*)mem::malloc(sizeof(T) * m_buffer_size);
        }
    }
...
public:
    ...
    /// Ctor with size hint.
    explicit HashMap(int size_hint)
        : entry_count(0)
        , size_mask(-1)
        , table(NULL)
    {
        setCapacity(size_hint);
    }
    ...
    struct Entry
    {
        ...
        bool isEmpty() const
        {
            return next_in_chain == -2;
        }
        bool isEndOfChain() const
        {
            return next_in_chain == -1;
        }
...
private:
    ...
    int findIndex(const T& key) const
    {
        ...
        if (e->isEmpty())
            return -1;
        if (int(e->hash_value & size_mask) != index)
