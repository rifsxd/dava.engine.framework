...
public:
    ...
    Color64(const Color64& c)
        : u(c.u)
    {
    }
    Color64(uint16 R, uint16 G, uint16 B, uint16 A)
    {
        setRGBA(R, G, B, A);
    }
    explicit Color64(uint64 U)
        : u(U)
    {
    }
...
public:
    ...
    Color32(uint8 R, uint8 G, uint8 B)
    {
        setRGBA(R, G, B, 0xFF);
    }
