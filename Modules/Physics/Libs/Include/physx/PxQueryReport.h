...
struct PxHitBuffer : public PxHitCallback<HitType>
{
    ...
    PxHitBuffer(HitType* aTouches = NULL, PxU32 aMaxNbTouches = 0)
        : PxHitCallback<HitType>(aTouches, aMaxNbTouches)
    {
    }
    ...
    {
        return getNbTouches() + PxU32(this->hasBlock);
    }
...
struct PxRaycastBufferN : public PxHitBuffer<PxRaycastHit>
{
    PxRaycastHit hits[N];
    PxRaycastBufferN()
        : PxHitBuffer<PxRaycastHit>(hits, N)
    {
    }
};
...
struct PxOverlapBufferN : public PxHitBuffer<PxOverlapHit>
{
    PxOverlapHit hits[N];
    PxOverlapBufferN()
        : PxHitBuffer<PxOverlapHit>(hits, N)
    {
    }
};
