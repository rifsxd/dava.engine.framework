...
struct PxQueryFilterData
{
    ...
    explicit PX_INLINE PxQueryFilterData()
        : flags(PxQueryFlag::eDYNAMIC | PxQueryFlag::eSTATIC)
        , clientId(PX_DEFAULT_CLIENT)
    {
    }

    /** \brief constructor to set both filter data and filter flags */
    explicit PX_INLINE PxQueryFilterData(const PxFilterData& fd, PxQueryFlags f)
        : data(fd)
        , flags(f)
        , clientId(PX_DEFAULT_CLIENT)
    {
    ...
    explicit PX_INLINE PxQueryFilterData(PxQueryFlags f)
        : flags(f)
        , clientId(PX_DEFAULT_CLIENT)
    {
    }
    ...
    PX_DEPRECATED PxClientID clientId; //!< ID of the client doing the query (see #PxScene.createClient())
};
