...
public:
    ...
    PX_CUDA_CALLABLE PX_FORCE_INLINE explicit PxTransform(const PxVec3& position)
        : q(PxIdentity)
        , p(position)
    {
    }
    ...
    PX_CUDA_CALLABLE PX_FORCE_INLINE explicit PxTransform(const PxQuat& orientation)
        : q(orientation)
        , p(0)
    {
