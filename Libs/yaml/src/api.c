...
{
	int sizeOfOldBuffer = (*end - *start);
	int sizeToPointer = (*pointer - *start);
	yaml_char_t *new_start = yaml_realloc(*start, sizeOfOldBuffer * 2);

	if (!new_start) return 0;

	memset(new_start + sizeOfOldBuffer, 0, sizeOfOldBuffer);

	*pointer = new_start + sizeToPointer;
	*end = new_start + sizeOfOldBuffer * 2;
	*start = new_start;
