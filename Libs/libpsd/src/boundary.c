	...
	psd_double line_width, psd_double flatness)
{
	...
	psd_int closed;
	psd_int last, this_, next, second;
	psd_double dx, dy;
	...
	for (begin_idx = 0; vpath[begin_idx].code != PSD_ART_END; begin_idx = end_idx)
	{
		...
		for (i = this_ + 1; vpath[i].code == PSD_ART_LINETO; i++)
		{
...
{
	const psd_art_svp_seg* seg1 = (const psd_art_svp_seg*)s1;
	const psd_art_svp_seg* seg2 = (const psd_art_svp_seg*)s2;
...
static void psd_art_svp_intersect_add_seg(psd_art_intersect_ctx *ctx, const psd_art_svp_seg *in_seg)
{
	...
	left = psd_art_svp_intersect_add_point(ctx, x0, y0, last, (psd_art_break_flags)(PSD_ART_BREAK_LEFT | PSD_ART_BREAK_RIGHT));
	seg->left = left;
	if (left == NULL)
	{
		right = ctx->active_head;
	...
	psd_art_pri_point *pri_pt)
{
	...
	if (in_curs + 1 == in_seg->n_points)
	{
		...
		if (left != NULL && right != NULL)
			...
			(psd_art_break_flags)(PSD_ART_BREAK_LEFT | PSD_ART_BREAK_RIGHT));
		psd_free(pri_pt);
	}
	else
	{
			...
			seg->points[curs].y < y + 1)
		{
			...
			if (y_top != y_bot)
			{
				...
				else
				{
					...
					for (; x < ix_max; x++)
					{
						this_ = (psd_int)((seg->dir ? 16711680.0 : -16711680.0) * rslope *
							(x + 0.5 - x_min));
						xdelta = this_ - last;
						last = this_;
